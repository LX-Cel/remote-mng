...
首版采用 Python 3.11+，使用 AsyncSSH、telnetlib3 和官方 MCP SDK，协议代码与应用状态分离；具体依赖和许可证见 [依赖说明](dependencies.md)。

| 候选 | 对本项目的主要收益 | 主要代价 | 本版决策 |
| --- | --- | --- | --- |
| Python | 可以复用目前已验证的 SSH、Telnet 和 MCP 实现，便于测试协议和进程异常 | 本地需要 Python 运行时；依赖需锁定 | 采用 |
| Go | 有利于后续分发单一可执行文件 | 需要重做本版已验证的 Telnet、交互与 MCP 组合 | 保留为后续打包/重写评估，不为首版增加双实现 |
| TypeScript/Node | 适合 MCP 与后续网页界面协作 | 本版没有网页端，切换会增加终端及进程管理验证工作 | 本版未采用 |
...
| `session` | 持续终端，可进入用户应用前台 | 实际收到的终端输出；没有通用逐命令退出码 | daemon 存活时可换客户端；原连接丢失后保留历史但不能恢复原终端 |
...
- 本地日志按 UTF-8 保存，每条流默认最多 64 MiB，达到上限后仍排空终端但停止保存新增字节，并设置 `log_truncated`。
...
本地记录和远端作业目录没有自动删除策略。远端原始请求可能含用户显式传入的环境值，原始日志可能含业务秘密，需使用当前用户的私有持久目录。工具不会擅自清理证据。
...
首版可复制的验证覆盖 Windows 原生与 WSL Ubuntu。Linux 进程及 Shell 语义在 WSL 内实测，另有 BusyBox applets 测试；独立 Linux 主机、厂商嵌入式设备及用户真实终端的现场结果仍需补充。详见 [验证报告](validation.md)。
